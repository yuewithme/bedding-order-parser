"""Write dictionary preview JSON safely."""
    ...
    *,
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            json.dump(bundle.to_json_dict(), temp_file, ensure_ascii=False, indent=2)
            temp_file.write("\n")
        os.replace(temp_name, resolved)
    except Exception as exc:
        if temp_name:
            Path(temp_name).unlink(missing_ok=True)
        raise OutputFileError(f"Failed to write dictionary preview: {resolved}") from exc
